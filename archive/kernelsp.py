...
class rixs_model(object):
        ...
        if self.nmodes==1:
            def func_temp(f):
                if self.dict['method']=='fc':
                    if self.dict['type_calc']=='1d':
                        return abs(self.amplitude(f,self.i))**2
                    elif self.dict['type_calc']=='dd':
                        return abs(self.amplitude_dd(f,self.i))**2
                    elif self.dict['type_calc']=='2d':
                        return abs(self.amplitude(f,self.i))**2
                    else:
                        print('error in kernelsp')
                else:
                    if self.dict['problem_type']=='rixs':
                        return abs(self.amplitude_gf(f))
                    elif self.dict['type_problem']=='rixs_q':
    ...
    def greens_func_cumulant(self):
        ...
        omegaq=self.dict['omega_ph0']*2*np.pi
        G0x=-1.j*np.exp(1.j*(np.pi*2.*self.dict['omega_in'])*t)
        Fx=np.exp(self.dict['g0']*(np.exp(1.j*omegaq*t)-1.j*omegaq*t-1))
        G=G0x*Fx*np.exp(-2*np.pi*self.dict['gamma']*t)
        GW=np.fft.fft(G)
...
#         self.q=np.linspace(-1,1,self.dict['nq'])
...
#         self.gkqx=self.gx(self.dict['qx'])
#         self.omegaqx=self.omegax(self.dict['qx'])
...
#         t=np.linspace(0.,self.dict['maxt'],self.dict['nstep'])
#         self.omegaq_cumulant=self.omegaq*2*np.pi
#         Fkq=1.
#         for gkqi,omegaqi in zip(self.gkq,self.omegaq_cumulant):
#             Fkq=Fkq*np.exp(gkqi*(np.exp(-1.j*omegaqi*t)+1.j*omegaqi*t-1)/self.dict['nq'])
#         return Fkq
#
