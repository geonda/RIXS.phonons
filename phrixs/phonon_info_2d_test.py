...
class energy(object):
	...
	def plot_colormap(self):
		...
		ims=ax.imshow(self.grid_rect_z.T, extent=(-max(self.q2d['x']),max(self.q2d['x'])\
			,-max(self.q2d['y']),max(self.q2d['y'])),origin='lower',cmap="plasma")
		plt.ylim(-1.5,1.5)
		...
		ax.scatter(-np.array(self.x),-np.array(self.y))
	...
	def interpolate(self,x,y,z):
		...
		points_neg=np.vstack((-x,-y))
		points_pos=np.vstack((x,y))
		points=np.hstack((points_neg,points_pos))
		values=np.hstack((z,z))
		# print(values.size,points.size)
		# print(max(x),max(y))
		self.grid_rect_x, self.grid_rect_y = np.mgrid[-max(x):max(x):11j, -max(y):max(y):11j]
		self.grid_rect_z = griddata(points_pos.T,z,(self.grid_rect_x, self.grid_rect_y), method='cubic')
