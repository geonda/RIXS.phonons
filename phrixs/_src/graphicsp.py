...
class graph(object):
    ...
    def simple_matplot(self,scale=1):
        ...
        plt.show()
    def simple_and_exp(self,file=[],scale=1,names_list=[]):
            self.fig = plt.figure(figsize=(7,5))
            self.p = self.fig.add_subplot(1, 1, 1)
            print('nruns:',self.nruns)
            for irun in range(self.nruns):
                if scale==0:
                    scale=1/max(self.dict_y[irun][:])
                try:
                    label=names_list[irun]
                except :
                    label=''
                self.p.plot(self.dict_x[irun],self.dict_y[irun][:]*scale,\
                        linewidth=2,color=self.color_list[irun],label=label)
                scale=0
            self.p.set_xlabel("$\mathrm{Energy\ Loss, \ eV}$",fontsize=15)
            self.p.set_ylabel("$\mathrm{RIXS\ Intensity, \ arb.\ units}$",fontsize=15)
            # self.p.set_xlim([0.,0.36])
            self.xexp,self.yexp=np.loadtxt('./_exp_files/'+file)
            self.p.scatter(self.xexp,self.yexp/max(self.yexp), facecolor='None',edgecolor='k',label='exp')
            self.p.set_xlim([-0.1,0.98])
            plt.legend()
            plt.show()
    ...
    def plot_rixsq_matplot_exp(self,scale=1,file=1):
        ...
        self.xtemp,self.ytemp=np.load('./_out/rixs_temp.npy')
