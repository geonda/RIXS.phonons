...
class workspace(object):
    ...
    def initp(self,type_problem='rixs',method='fc',type_calc='model',\
        ...
        self.dict_problem=init_problem(type_problem,method,type_calc,\
                                                        el_space, vib_space)
    ...
    def runp(self):
        self.nruns+=1
        rixs_model(nruns=self.nruns).cross_section()
        spectra(nruns=self.nruns).run_broad()
    def plotp(self):
        pg.mkQApp()
        ...
        [graph(self.app,self.win,self.plot,nruns=runs+1).simple()\
