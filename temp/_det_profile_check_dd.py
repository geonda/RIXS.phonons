...
detuning=np.linspace(-0.6,0.6,40) # for beta 1 detuning step can be changed
...
for i,beta in enumerate(beta_list):
    ...
    for dE in detuning:
        ...
        intesity_of_elastic_peak[beta].append(data_temp[1][0])
    plt.subplot(2,1,1)                                          # first phonon peak intensity
    plt.plot(detuning,intesity_of_elastic_peak[beta],
                            '-',label='0 ph vs det',color='b',alpha=al[i])
    plt.plot(detuning,intesity_of_first_peak[beta],
                        '-',label='1 ph vs det',color='r',alpha=al[i])

    plt.axvline(0,color='grey',linestyle='--')
    arr=np.array(intesity_of_elastic_peak[beta])
    result = np.where(arr == np.amax(arr))
    plt.axvline(detuning[result[0]],color='green',linestyle='-')
    plt.legend()
    ...
    plt.subplot(2,1,2)                                          # first phonon peak intensity
